...
migration transition for each configured evolution step; toggles topology,
plate, crust, motion, and final-boundary diagnostics; and reports aggregate
evolution statistics and stage timings. Plate interiors use stable per-plate
colors. Crust is blue for oceanic, amber for continental, and white where the
classes meet. Motion arrows sample each plate's local tangent velocity field.
Static boundaries are red for convergent, blue for divergent, and yellow for
transform.
