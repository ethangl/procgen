@group(0) @binding(0) var<storage, read> terrain_control_texels: array<TerrainControlTexel>;
...
@group(0) @binding(5) var<uniform> terrain_dispatch: TerrainDispatch;
...
fn generate_terrain_tiles(@builtin(global_invocation_id) id: vec3<u32>) {
    let sample_count = terrain_dispatch.tile_count * 4225u;
    if id.x >= sample_count { return; }
    let slot = id.x / 4225u;
    let local_index = id.x % 4225u;
    let local = vec2(local_index % 65u, local_index / 65u);
