...
struct HeightVertex { anchor: vec4<i32>, offset: vec4<f32> }
...
fn height_mesh(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    let footprint = max(pilot_sqrt(pilot_length_squared(delta)+height_filter.w*height_filter.w)/HEIGHT_FILTER_DISTANCE_RATIO,HEIGHT_FILTER_MIN_M);
    ...
    height_vertices[id.x] = HeightVertex(vec4(anchor, i32(tile.y)), vec4(residual + direction * height, spacing));
}
