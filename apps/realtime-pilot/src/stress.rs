    ...
    for NamedCase { name, case } in cases {
        ...
        match evaluate(case.scenario) {
            Ok(r) => {
                ...
                row["metrics"] = serde_json::json!({"preparation_ms":r.preparation_ms,"mesh_checks_ms":r.mesh_checks_ms,"walk_ms":r.walk_ms,"walk_step_max_ms":r.walk_step_max_ms,"triangles":r.triangles,"nonmanifold_edges":r.topology.nonmanifold_edges,"radius_range":r.radius_range,"mesh_fingerprint":format!("{:016x}",r.mesh_fingerprint),"route_fingerprint":format!("{:016x}",r.route_fingerprint),"placements":r.placements,"min_clearance":r.min_clearance,"walk_displacement":r.walk_displacement,"rest_drift":r.rest_drift,"source_bytes":r.source_bytes,"collision_index_bytes":r.collision_index_bytes});
            }
            Err(e) => {
                failed += 1;
