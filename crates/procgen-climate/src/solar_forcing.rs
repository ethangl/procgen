...
pub struct SolarForcingDiagnostics {
    ...
    /// Daily-mean insolation statistics in watts per square meter.
    pub daily_mean: AreaWeightedSummary,
    /// Annual-mean insolation statistics in watts per square meter.
...
impl SolarForcing {
    pub fn validate(&self, mesh: &SphereMesh) -> Result<(), SolarForcingError> {
        if self.daily_mean_insolation.len() != mesh.cell_count()
            || self.annual_mean_insolation.len() != mesh.cell_count()
        {
            return Err(SolarForcingError::Cells);
        }
        if self
            .daily_mean_insolation
            .iter()
            .chain(&self.annual_mean_insolation)
            .any(|value| !value.is_finite() || *value < 0.0)
        {
            return Err(SolarForcingError::Insolation);
        }
        Ok(())
    }
}
...
impl fmt::Display for SolarForcingError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::Cells => formatter.write_str("solar-forcing fields must match the mesh cells"),
            Self::Insolation => {
...
pub fn derive_solar_forcing(
    ...
    Ok(SolarForcing {
        diagnostics: SolarForcingDiagnostics {
            ...
            daily_mean: AreaWeightedSummary::from_field(mesh, &daily_mean_insolation),
            annual_mean: AreaWeightedSummary::from_field(mesh, &annual_mean_insolation),
        },
        daily_mean_insolation,
...
mod tests {
    ...
    fn zero_obliquity_matches_equatorial_daily_mean_and_global_energy() {
        ...
        assert!((forcing.diagnostics.daily_mean.area_weighted_mean - 0.25).abs() < 2.0e-3);
        assert_eq!(
