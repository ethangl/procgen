...
//!
//! Deform, transport, drift, lifecycle, and then the run reclassifies. A step
//! raises deformation along the boundaries it starts from, moves every
//! particle of crust with its plate and resolves what each cell then holds,
//! drifts every plate's rotation vector, and rifts and sutures plates. The
//! deformation happens first because uplift happens at the boundary and the
//! material moves afterwards; the drift happens third, so the step's two
...
//! last, so the next classification reads the new plate set.
//!
...
pub struct PoleDriftConfig {
    ...
    /// multiplies a plate's speed by `1 + s * rate * sqrt(step_duration)` for
    /// a hashed `s` in `[-1, 1)`.
    pub speed_drift_rate: f32,
    /// Fraction either side of the speed a plate started with that its
    /// drifted speed may reach. The band is relative to the plate's own
    /// starting speed rather than to the global angular-speed range, because
    /// a random walk against fixed global limits eventually piles every plate
    /// against one of them, where a band around where the plate began keeps
    /// drift the perturbation of it that it is meant to be. At one the band
    /// reaches zero; a plate can slow to a stop but never reverse.
...
impl Default for PoleDriftConfig {
    fn default() -> Self {
        Self {
            ...
            axis_drift_rate: 1.8,
            // `0.9 * sqrt(0.014)` is 0.106, so a step changes a plate's
            // speed by at most about a tenth.
...
mod tests {
    ...
    /// The band is around the speed the plate started the run with, which is
    /// what keeps a long walk a perturbation of the fit rather than a drift
    /// into the global limits.
    #[test]
    fn a_long_walk_leaves_every_speed_inside_its_band() {
        let fixture = two_plate_fixture(-1.0, vec![CrustClass::Continental, CrustClass::Oceanic]);
        let config = drift_config(1);
        let limit = config.pole_drift.speed_drift_limit;
        let mut world = EvolvingWorld::new(&fixture.mesh, fixture.inputs(), config);
        let started = world.starting_speeds.clone();
        assert!(started.iter().all(|&speed| speed > 0.0));
        ...
        for step in 0..200 {
            ...
            for (plate, rotation) in world.kinematics.angular_velocities.iter().enumerate() {
                let speed = rotation.length();
                let bounds =
                    started[plate] * (1.0 - limit) - slack..=started[plate] * (1.0 + limit) + slack;
                assert!(
                    bounds.contains(&speed),
                    "plate {plate} left its band at step {step}: {speed} against {bounds:?}"
                );
                reached = reached.max((speed / started[plate] - 1.0).abs());
            }
        }
