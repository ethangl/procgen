...
use procgen_sphere_mesh::{PointLocationError, SphereMesh, build_sphere_mesh};
...
fn assert_reconstructs(mesh: &SphereMesh, direction: Vec3, hint: Option<usize>) -> usize {
    let location = mesh.locate_delaunay(direction, hint).unwrap();
...
fn locates_cell_centers_with_one_hot_weights() {
    ...
    for cell in 0..mesh.cell_count() {
        ...
        for hint in [0, mesh.vertex_count() / 2, mesh.vertex_count() - 1] {
            let location = mesh.locate_delaunay(direction, Some(hint)).unwrap();
            assert_eq!(
...
fn locates_interiors_from_arbitrary_starting_hints() {
    ...
    assert_reconstructs(&mesh, direction, Some(mesh.vertex_count() - 1));
}
...
fn shared_boundaries_have_hint_independent_ownership() {
    ...
    for hint in [0, triangle, neighbor, mesh.vertex_count() - 1] {
        let location = mesh.locate_delaunay(edge_direction, Some(hint)).unwrap();
        assert_eq!(location.triangle, expected, "hint {hint}");
        assert!(location.weights.contains(&0.0));
        assert_reconstructs(&mesh, edge_direction, Some(hint));
    }
}

#[test]
fn validates_direction_and_hint() {
    let mesh = mesh(32);
    assert_eq!(
        mesh.locate_delaunay(Vec3::new(f32::NAN, 0.0, 0.0), None),
        Err(PointLocationError::NonFiniteDirection)
    );
    assert_eq!(
        mesh.locate_delaunay(Vec3::ZERO, None),
        Err(PointLocationError::DirectionNotOnUnitSphere { length: 0.0 })
    );
    assert_eq!(
        mesh.locate_delaunay(Vec3::new(2.0, 0.0, 0.0), None),
        Err(PointLocationError::DirectionNotOnUnitSphere { length: 2.0 })
    );
    assert_eq!(
        mesh.locate_delaunay(Vec3::new(1.0, 0.0, 0.0), Some(mesh.vertex_count())),
        Err(PointLocationError::InvalidTriangleHint {
            hint: mesh.vertex_count(),
            triangle_count: mesh.vertex_count(),
        })
    );
}
...
fn repeated_queries_are_deterministic() {
    ...
    for triangle in (0..mesh.vertex_count()).step_by(37) {
        ...
        let expected = mesh.locate_delaunay(direction, Some(triangle)).unwrap();
        for hint in [0, mesh.vertex_count() / 2, mesh.vertex_count() - 1] {
            assert_eq!(mesh.locate_delaunay(direction, Some(hint)), Ok(expected));
        }
    }
}
    ...
    for triangle in (0..mesh.vertex_count()).step_by(997) {
        ...
        assert_eq!(
            assert_reconstructs(
                &mesh,
                direction,
                Some((triangle + 31_337) % mesh.vertex_count())
            ),
            triangle
        );
    }
