...
- A config field is model time, model length or angle on the unit sphere,
  ...
  `TRANSPORT_REACH_HOPS`, `MaterialTransportConfig::gap_radius`, and
  `CrustBirthPriorConfig::ridge_less_age` are the named exceptions, because
  they describe the raster rather than the world. A length default is written
  as a multiple of `procgen_sphere_mesh::default_hop_length`, and the stage
  converts it once through `hops`.
