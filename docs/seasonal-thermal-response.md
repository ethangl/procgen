...
elevation strictly above the sea-level datum the elevation field carries
selects the land capacity; elevation at or below it selects the ocean capacity.
This is the shared tectonic elevation predicate used elsewhere in the pipeline,
and the datum reaches this stage as an explicit input rather than as a
constant. The stage does not inspect tectonic crust classes.
